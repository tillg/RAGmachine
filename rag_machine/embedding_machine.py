...
class EmbeddingMachine:
    ...
    def __init__(self, data_path: str = EMBEDDINGS_DATA_PATH, collection: str = EMBEDDINGS_COLLECTION, chunk_machine = None):
        ...
        logger.info(f"EmbeddingMachine initialized. Data path: {self.data_path}, collection size: {self.collection.count()}")
    ...
    def search_documents_by_texts(self, query_texts: List[str]):
        return emself.collection.query(query_texts=query_texts)

    def add_documents(self, documents: List[Document]):
        chunks = self.chunk_machine.chunkify_documents(documents)
        chunk_documents = [chunk.content for chunk in chunks]
        chunk_metadatas = [{"document_id": chunk.document_id} for chunk in chunks]
        chunk_ids = [chunk.id for chunk in chunks]
        self.collection.add(documents = chunk_documents, metadatas = chunk_metadatas, ids = chunk_ids)
